...
export default function Dashboard() {
  ...
  const loadProfile = async () => {
    try {
      const res = await authedFetch(`${API_URL}/api/accounts/profile/`, {
        method: "GET",
      });
  ...
  const loadIssues = async () => {
    ...
    try {
      const url =
        role === "provider"
          ? `${API_URL}/api/issues/?profession=${profession}`
          : `${API_URL}/api/issues/`;
  ...
  const handleLike = async (id) => {
    try {
      const res = await authedFetch(`${API_URL}/api/issues/${id}/like/`, {
        method: "POST",
      });
      if (!res.ok) return;
  ...
  const handleClaim = async (id) => {
    try {
      const res = await authedFetch(`${API_URL}/api/issues/${id}/claim/`, {
        method: "POST",
      });
      ...
      {loading || profileLoading ? (
        ...
        <Row>
          {issues.map((issue) => {
            const youAssigned =
              issue.assigned_to &&
              (issue.assigned_to.username === username ||
